...
public class SongsActivity extends FilteredLazyListActivity<ItemTO> {
    ...
    protected URLTaskParamTO createCountTaskParamTO(String filter) {
        return new URLTaskParamTO(Config.SONGS_COUNT_RESOURCE + "?filter=" + filter);
    }

    @Override
    protected URLTaskParamTO createFetchTaskParamTO(String filter, int pageSize, int page) {
        return new URLTaskParamTO(Config.SONGS_LIST_RESOURCE + "?pageSize=" + pageSize + "&page=" + page + "&filter=" + filter);
    }
